class Tableau<T> {
    ...
    filter(callback: (value: T, index: number, array: Array<T>) => boolean): Tableau<T> {
        const tab = new Tableau<T>()
        for (let i = 0; i < this.tab.length; i++) {
            if (callback(this.tab[i], i, this.tab)) {
                tab.push(this.tab[i])
            }
    ...
    map(callback: (value: T, index: number, array: Array<T>) => T | void): Tableau<T> {
        const tab = new Tableau<T>()
        for (let i = 0; i < this.tab.length; i++) {
            const result = callback(this.tab[i], i, this.tab)
            tab.push(result)
        }
        return tab
    }
    reduce(callback: (accumulator: T | undefined, currentValue: T) => T | undefined): T | undefined {
        if (this.tab.length === 0) {
            throw TypeError('Reduce of empty array with no initial value')
        } else if (this.tab.length === 1) {
            return this.tab[0]
        }
        let acc: T | undefined = this.tab[0]
