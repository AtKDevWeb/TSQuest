 ...
 */
let x = "hello world"; //🚨 ERROR : Type 'string' is not define.
 ...
 */
 let z; //type any
 z = 41; //z become an int 
 z = "abc"; // (6) oh no! This isn't good >> //🚨 ERROR : z:number become a Type 'string' because the varType is any.  
...
let aa: number[] = [];
aa.push(33);
aa.push("abc");//🚨 ERROR : Type 'array: string' is not assignable to type 'array: number'
 ...
 */
 ...
 bb = [1, 2, 3];//🚨 ERROR : Type 'an arrayElement:number'  is not assignable on an arrayElement: 'string'


/**
 * (10) Tuple values often require type annotations (  : [number, number] )
 */
 const xx = [32, 31]; // number[]; create a table of Constatnte Var Types 32 & 33 
 const yy: [number, number] = [32, 31]; //create a table of Constatnte Var Types number of value 32 & 33 
 ...
 */
 ...
 cc = {
   houseNumber: 33 // //🚨 ERROR : The object is incomplet : we wait 2 value One Number & One String define on the interface.
 };
 ...
 */
 let otherContactInfo: HasEmail & HasPhoneNumber = { //🚨 ERROR : the intersection type isn't possible.
